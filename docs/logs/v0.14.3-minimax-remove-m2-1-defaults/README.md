# v0.14.3-minimax-remove-m2-1-defaults

## 迭代完成说明

- 从 MiniMax 内置 provider 的默认模型列表中移除 `2.1` 这一代：
  - 删除 `minimax/MiniMax-M2.1`
  - 删除 `minimax/MiniMax-M2.1-highspeed`
- 保留的默认模型列表收敛为：
  ...
  - `minimax/MiniMax-M2`
- 同步更新 provider meta 测试断言，确保 UI 不再读到 `2.1` 系列默认模型。
...
- API 冒烟：
  ...
  - 观察点：`minimax.defaultModels` 只包含 `M2.7 / M2.7-highspeed / M2.5 / M2.5-highspeed / M2`
- 可维护性自检：
...
- 结果：
  ...
  - API 冒烟返回 `status=200`，且默认模型列表不再包含 `2.1` 系列
  - 可维护性检查无阻塞项，仅有接近预算线警告

## 发布/部署方式

- 本次改动仅涉及内置 provider 默认模型元数据与测试，无独立部署步骤。
- 随后续常规版本发布进入产物即可，不涉及 migration。

## 用户/产品视角的验收步骤

1. 打开 NextClaw UI 的 Provider 配置页，选择 `MiniMax`。
2. 确认默认模型列表中不再出现 `MiniMax-M2.1` 与 `MiniMax-M2.1-highspeed`。
3. 确认仍可见：
   - `MiniMax-M2.7`
