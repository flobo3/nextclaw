...
- 访问与调用约定已拆分至独立文件：[ACCESS.md](./ACCESS.md)。
- `ITERATION.md` 保留策略与设计主线，`ACCESS.md` 聚焦接入方实际调用（域名、鉴权、接口、错误码、示例）。
...
2. 管理员创建一个平台模型（如 `nextclaw/gpt-4o`），并配置售价与路由候选。
