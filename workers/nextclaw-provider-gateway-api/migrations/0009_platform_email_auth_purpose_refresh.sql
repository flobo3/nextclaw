...
CREATE TABLE platform_email_auth_codes_v2 (
  id TEXT PRIMARY KEY,
  email TEXT NOT NULL,
...
DROP TABLE platform_email_auth_codes;
ALTER TABLE platform_email_auth_codes_v2 RENAME TO platform_email_auth_codes;

CREATE INDEX idx_platform_email_auth_codes_lookup
  ON platform_email_auth_codes(email, purpose, created_at DESC, id DESC);

CREATE INDEX idx_platform_email_auth_codes_session
  ON platform_email_auth_codes(browser_auth_session_id, created_at DESC, id DESC);

CREATE INDEX idx_platform_email_auth_codes_active
